...
while True:
    try:
        ...
        gray7 = 224
        white = 255
        smallFont = ImageFont.truetype(os.path.join(picdir, 'Font.ttc'), 15)
        mediumSmallFont = ImageFont.truetype(os.path.join(picdir, 'Font.ttc'), 18)
        ...
        for day in days:
            ...
            draw.text((((index + 1) * 171) - (171 / 2), 1), days[index], font = mediumSmallFont, fill = black)
            index = index + 1
        ...
        while i < 15:
            ...
            draw.text((cellLeft + 2, cellTop - 2), str(currentDay.day), font = mediumFont, fill = black)
            cursorTop = cellTop + 30
            cursorLeft = cellLeft + 5
            while shiftIndex < len(shifts):
                ...
                if shiftDate.day == currentDay.day and shiftDate.month == currentDay.month:
                    ...
                    if endhours > 12:
                        ...
                        endtimeofday = 'AM'
                    if shift.get('workerFullName', False):
                        temp = shift['workerFullName'].split()
                        text1 = temp[0] + ' ' + temp[1][0:1]
                    else:
                        text1 = 'UNFILLED'
                    text2 = text1 + ' - ' + shift['businessPositionName'] + ' ' + str(starthours) + ':' + (str(startminutes) if startminutes else '00') + starttimeofday + ' - ' + str(endhours) + ':' + (str(endminutes) if endminutes else '00') + endtimeofday
                    lines = textwrap.wrap(text2, 20)
                    if text1 == 'UNFILLED':
                        fill = white
                    else:
                        fill = gray7
                    draw.rectangle([(cursorLeft - 2, cursorTop), (cellRight - 2, cursorTop + (len(lines) * 18))], fill = fill, outline=0)
                    for line in lines:
                        draw.text((cursorLeft, cursorTop), line, font = smallFont, fill = black)
                        cursorTop += 18
                    cursorTop = cursorTop + 5
                shiftIndex += 1
